const GRADES = [
  { min: 80, emoji: '🏆', label: 'Excellent!', color: 'text-amber-500' },
  { min: 60, emoji: '🎯', label: 'Good job!', color: 'text-blue-500' },
  { min: 40, emoji: '📚', label: 'Keep learning!', color: 'text-violet-500' },
  { min: 0, emoji: '💪', label: 'Try again!', color: 'text-slate-500' },
];
...
export function FinishedScreen({ score, total, onReset }) {
  ...
  return (
    <div className="flex flex-col items-center p-12 bg-white border border-slate-200 rounded-2xl max-w-md mx-auto text-center">
      ...
      <div className="flex gap-6 mb-8">
        <div>
          ...
          <p className="text-xs text-slate-400 uppercase tracking-wide font-semibold">Correct</p>
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 uppercase tracking-wide font-semibold">Score</p>
        </div>
      </div>
